...
class GridStorage(object):
  ...
  Attributes:
    ...
    discharge_nameplate_cost: A float which is the cost per nameplate
      power to recharge the ge.  E.g. The cost of a power inverter to
...
class LinearProgramContainer(object):
  ...
  def _post_process(self):
    ...
    if sum_rps_credits < total_demand * self.rps_percent / 100:
...
def extrapolate_cost(cost, discount_rate, time_span_1, time_span_2):
  ...
  denominator = (1.0-value_decay_2)

  if denominator == 0:
    return cost
  else:
    return cost * (1.0 - value_decay_1) / denominator


def _build_sources_from_protobufs(source_attributes):
  """Builds GridSources from source protobufs.

  Args:
    source_attributes: repeated SourceAttribute protobuf.
  Returns:
    list of GridSources
  """
  ...
  for source in source_attributes:
    ...
    source_args = {
        'name': source.name,
        'nameplate_unit_cost': source.capital,
        'variable_unit_cost': source.variable,
        'grid_region_id': source.grid_region_id,
        'max_power': source.max_power,
        'max_energy': source.max_energy,
        'co2_per_electrical_energy': source.co2_per_electrical_energy,
        'power_coefficient': 1.0,
        'is_rps_source': source.is_rps_source
    }

    grid_sources.append(GridSource(**source_args))

  return grid_sources


def solve_linear_program_from_protobuf(grid_configuration,
                                       profiles_df,
                                       populate_solutions=False):
  """Runs linear_program solver.

  Args:
    grid_configuration: GridConfiguration protobuf.
    profiles_df: dataframe of timeslice profiles.
    populate_solutions: Boolean: If true, time-slice solution values are
      populated in results protobufs

  Returns:
    grid_sim_proto:Results protobuf.

  """

  lp = LinearProgramContainer(profiles_df)

  # Marshal source protobufs into GridSource objects.
  dispatchable = _build_sources_from_protobufs(
      grid_configuration.dispatchable_sources)

  lp.add_dispatchable_sources(*dispatchable)

  non_dispatchable = _build_sources_from_protobufs(
      grid_configuration.nondispatchable_sources)

  lp.add_nondispatchable_sources(*non_dispatchable)

  # Marshal demand protobufs into GridSource objects.
  lp.add_demands(*[GridDemand(d.name, d.grid_region_id)
                   for d in grid_configuration.demands])

  for storage in grid_configuration.storage:
    storage_args = {
        'name': storage.name,
        'storage_nameplate_cost': storage.capital,
        'charge_nameplate_cost': storage.charge_capital,
        'discharge_nameplate_cost': storage.discharge_capital,
        'grid_region_id': storage.grid_region_id,
        'charge_efficiency': storage.charge_efficiency,
        'storage_efficiency': storage.storage_efficiency,
        'discharge_efficiency': storage.discharge_efficiency,
        'max_charge_power': storage.max_charge_power,
        'max_discharge_power': storage.max_discharge_power,
        'max_storage': storage.max_storage,
    }
    lp.add_storage(GridStorage(**storage_args))

  lp.carbon_tax = grid_configuration.carbon_tax
  lp.cost_of_money = extrapolate_cost(1.0,
                                      grid_configuration.annual_discount_rate,
                                      len(profiles_df) /
                                      grid_configuration.timeslices_per_year,
                                      grid_configuration.lifetime_in_years)

  lp.rps_percent = grid_configuration.rps_percent

  results = gcp.Results()

  # Copy configuration into results.configuration.
  results.configuration.CopyFrom(grid_configuration)

  # Solve.
  lp.solve(results, populate_solutions)
